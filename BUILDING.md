...
- Install [sh4zam](https://github.com/gyrovorbis/sh4zam) from kos-ports when possible — the build auto-detects `libsh4zam.a` and enables `DC_HAS_SH4ZAM` for matrix math, fog blending, and audio volume scaling. Pass `-DDC_ENABLE_SH4ZAM=OFF` to force portable fallbacks.
- Global Dreamcast flags follow SM64 DC discipline: `-Os -flto=auto` with `-O3` on `dc_gbi`, `dc_pvr_renderer`, `dc_texture_cache`, and `dc_audio`.
- Optional fixed PVR textures: bake `.dt` files (see `tools/dreamcast/fixed_textures/README.md`) and stage them on disc under `/cd/fixed_textures/`.
- Compare unimplemented GBI opcodes against SM64 with `python3 tools/dreamcast/compare_gbi_dispatch.py`.
