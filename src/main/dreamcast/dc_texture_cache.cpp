...
uint32_t texture_height(const LoadedTexture& tex, const TileState& tile) {
    const uint32_t width = std::max<uint32_t>(texture_width(tex, tile), 1);
...
TexelColor sample_texel(
    ...
    uint32_t width = std::max<uint32_t>(texture_width(tex, tile), 1);
    uint32_t height = std::max<uint32_t>(texture_height(tex, tile), 1);
